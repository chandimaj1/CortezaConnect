...
/**
 * 
 *  Empty airports table and Upload demo content to the database
 */
function fetch_dataset(){
    $('#fetch_dataset').on('click', function(){
        $('#confirm_fetch').modal('show');
    });

    $('#proceed_fetch_dataset').on('click', function(){

        $('#confirm_fetch .modal-title').html('Processing... !');
        $('#confirm_fetch .modal-body').html('<h4 style="color:#c00">Please wait until the database is updated...</h5> This process might not be completed due to limitations in your php.ini settings. Please use manual database update methods if 10k+ records not retrieved.');
        $('#confirm_fetch .modal-footer').remove();

        $.ajax({     
            url: ajax_url + 'fetch_airports_dataset_csv.php',
            method: "GET",
            success: function(data)
            { 
                console.log(data);
                $('#confirm_fetch .modal-title').html('Success !');
