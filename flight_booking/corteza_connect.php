...
class CortezaConnect
{
    ...
    function add_admin_pages(){
        add_menu_page( 'VeloxJets Pricing - Settings', 'VeloxJets Pricing - Settings', 'manage_options', 'CortezaConnect_settings', array($this,'admin_index'), 'dashicons-list-view', 100);
    ...
    function create_table(){ 
        ...
        // aircrafts
        $table_name = $wpdb->prefix."CortezaConnect_aircrafts";
        ...
        if ($wpdb->get_var('SHOW TABLES LIKE '.$table_name) != $table_name) {
            ...
            VALUES (1,'charter@veloxaircharter.com','d51f7a015cbefc69520b41c2c2ca0803','http://data.fixer.io/api/latest')";
            require_once(ABSPATH.'wp-admin/includes/upgrade.php');
            dbDelta($sql);
            dbDelta($sql_insert);
            add_option("CortezaConnect_settings_db", "1.1");
        }

        // Theme
        $table_name = $wpdb->prefix."CortezaConnect_theme";
        if ($wpdb->get_var('SHOW TABLES LIKE '.$table_name) != $table_name) {
            $sql = 'CREATE TABLE '.$table_name.'(
                id int(11) NOT NULL,
            ...
            dbDelta($sql);
            dbDelta($sql_insert);
            add_option("CortezaConnect_theme_db", "1.1");
        }

        // Languages
        $table_name = $wpdb->prefix."CortezaConnect_languages";
        if ($wpdb->get_var('SHOW TABLES LIKE '.$table_name) != $table_name) {
            $sql = 'CREATE TABLE '.$table_name.'(
            id INTEGER NOT NULL AUTO_INCREMENT,
            language VARCHAR(25),
            ...
            ('2', 'spannish', 'One Way S', 'Round Trip', 'Multi Leg', 'Empty Leg', 'From', 'Where to?', 'Departure Date', 'Return Date', 'Passenger', 'SEARCH', 'Turboprop', 'Light Jets', 'Midsize Jets', 'Super MidsizeJets', 'Heavy Private Jets', 'VIP Airliners', 'Inquiry', 'Estimated price before taxes & fees.', 'Please provide your contact details here.', 'Name', 'Email', 'Phone Number', 'Any special requests or requirements', 'SEND INQUIRY', 'Please wait while we source the available aircraft', 'We cannot find any result for the current selection. Please make sure that required data is entered.'),
            ('3', 'portugese', 'One Way P', 'Round Trip', 'Multi Leg', 'Empty Leg', 'From', 'Where to?', 'Departure Date', 'Return Date', 'Passenger', 'SEARCH', 'Turboprop', 'Light Jets', 'Midsize Jets', 'Super MidsizeJets', 'Heavy Private Jets', 'VIP Airliners', 'Inquiry', 'Estimated price before taxes & fees.', 'Please provide your contact details here.', 'Name', 'Email', 'Phone Number', 'Any special requests or requirements', 'SEND INQUIRY', 'Please wait while we source the available aircraft', 'We cannot find any result for the current selection. Please make sure that required data is entered.');";
            require_once(ABSPATH.'wp-admin/includes/upgrade.php');
            dbDelta($sql);
            dbDelta($sql_insert);
            add_option("CortezaConnect_languages_db", "1.1");
        }

        // airports
        $table_name = $wpdb->prefix."CortezaConnect_airports";
        if ($wpdb->get_var('SHOW TABLES LIKE '.$table_name) != $table_name) {
            $sql = 'CREATE TABLE '.$table_name.'(
            id INTEGER NOT NULL AUTO_INCREMENT,
            ...
            require_once(ABSPATH.'wp-admin/includes/upgrade.php');
            $sql_insert = "INSERT INTO $table_name
            VALUES (1, 'Sample Airport Name', 'Sample Airport City', 'Sample Airport Country', 'IAT', 'ICAO', 0.0,'XX', 0.0, 0.0, 0)";
            dbDelta($sql);
            dbDelta($sql_insert);
            add_option("CortezaConnect_languages_db", "1.1");
        }
    ...
    function enqueue_admin($hook_suffix){
        ...
        if (strpos($hook_suffix, 'CortezaConnect_settings') !== false) {
            ...
            //Select2
            wp_enqueue_style( 'CortezaConnect_select2_styles', plugins_url('/assets/select2/select2.css',__FILE__));
            wp_enqueue_script( 'CortezaConnect_select2_scripts', plugins_url('/assets/select2/select2.full.js',__FILE__), array('jquery'));
        
        }else if (strpos($hook_suffix, 'CortezaConnect_airports_settings') !== false) {
            //Bootstrap
            wp_enqueue_style( 'bootstrap4_styles', plugins_url('/assets/bootstrap4/bootstrap_4_5_2_min.css',__FILE__));
            wp_enqueue_script( 'bootstrap4_scripts', plugins_url('/assets/bootstrap4/bootstrap_4_5_2_min.js',__FILE__), array('jquery'));
            //Font-Awesome
            ...
            wp_enqueue_style( 'CortezaConnect_airports_admin_styles', plugins_url('/assets/CortezaConnect_airports_admin_style.css',__FILE__));
            wp_enqueue_script( 'CortezaConnect_airports_admin_script', plugins_url('/assets/CortezaConnect_airports_admin_scripts.js',__FILE__), array('bootstrap4_scripts','jquery'));
        }
    }
    ...
    function enqueue(){ 
        ...
        //echo($post_slug);
        if ( $post_slug=="CortezaConnect" || true ){
        //Bootstrap
        wp_enqueue_style( 'bootstrap4_css', plugins_url('/assets/bootstrap4/bootstrap_4_5_2_min.css',__FILE__),80);
         ...
         //Select2
         wp_enqueue_style( 'CortezaConnect_select2_styles', plugins_url('/assets/select2/select2.css',__FILE__),98);
         wp_enqueue_script( 'CortezaConnect_select2_scripts', plugins_url('/assets/select2/select2.full.js',__FILE__), array('jquery'));
         //Fuse
        // wp_enqueue_script( 'airports_scripts', plugins_url('/assets/fuse.js/airports.js',__FILE__));
         ...
         //DateJS
         wp_enqueue_script( 'CortezaConnect_momentjs', plugins_url('/assets/momentjs/momentjs.js',__FILE__), array('jquery'));
         //Custom
         $color='gray';
         if ( $color=='gray' ){
            wp_enqueue_style( 'CortezaConnect_custom_styles', plugins_url('/assets/gray.css',__FILE__),100);
        }else if ( $color=='blue' ){
            wp_enqueue_style( 'CortezaConnect_custom_styles', plugins_url('/assets/blue.css',__FILE__),100);
        }

        wp_enqueue_style( 'CortezaConnect_common_styles', plugins_url('/assets/common.css',__FILE__),101);

         //FrontEnd scripts and styles
         wp_enqueue_script( 'CortezaConnect_script', plugins_url('/assets/CortezaConnect_scripts.js',__FILE__), array('CortezaConnect_select2_scripts','flatpickr_scripts','fuse_scripts','jquery'));
        }
    }
